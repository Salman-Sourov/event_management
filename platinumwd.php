	...
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["shopping_cart"] as $keys => $values)
		{
			if($values["item_id"] == $_GET["id"])
			{
				...
				echo '<script>window.location="platinumwd.php"</script>';
			}
		}
	}
}
...
<html>
	<head>
		<title>Weeding Gold Package</title>
	...
	<body>
      ...
      <nav class="navbar navbar-expand-md navbar-dark bg-dark">
		...
		<div class="container">
			<br />
			<span class="section-heading">Weeding Gold Package Details</span><br />
			<br />
...
<span class="section-heading-left">Stage Design</span>
			<?php
				...
				if(mysqli_num_rows($result) > 0)
				{
					...
					{
			?>			
			<form method="post" action="platinumwd.php?action=add&id=<?php echo $row["id"]; ?>">
				<div class="col-md-4">
			...
			<form method="post" action="platinumwd.php?action=add&id=<?php echo $row["id"]; ?>">
				<div class="col-md-4">
					...
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align="center">
						...
						<h4 class="text-info"> [ <?php echo $row["details"]; ?>Max Distance 20 KM ] </h4>
...
<span class="section-heading-left">Invitation Card</span>
			<?php
				$query = "SELECT * FROM wd_card ORDER BY id ASC";
				$result = mysqli_query($connect, $query);
				if(mysqli_num_rows($result) > 0)
				{
					while($row = mysqli_fetch_array($result))
...
<span class="section-heading-left">Catering Service</span>
			<?php
				$query = "SELECT * FROM wd_catering ORDER BY id ASC";
				$result = mysqli_query($connect, $query);
				if(mysqli_num_rows($result) > 0)
				{
					while($row = mysqli_fetch_array($result))
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					<tr>
						...
						<td><a href="platinumwd.php?action=delete&id=<?php echo $values["item_id"]; ?>"><span class="text-danger">Remove</span></a></td>
					</tr>
					<?php
